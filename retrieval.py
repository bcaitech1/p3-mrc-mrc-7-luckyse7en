...
class SparseRetrieval:
    def __init__(self, tokenize_fn, data_path="./data/", context_path="wikipedia_documents.json"):
        ...
        self.contexts = prepro.prepro(self.contexts)
...
#                sum_text = ''
#                for i in range(topk):
#                    sum_text += self.contexts[doc_indices[idx][i]] + ' '
                # 추가부분 끝
...
#                    "context": sum_text
# 변경 전                    "context": self.contexts[doc_indices[idx][0]]  # retrieved doument
                }
###
                if 'context' in example.keys():
                    tmp["context"] = example['context']+ ' '.join(self.contexts[i] for i in doc_indices[idx] if self.contexts[i] != example['context'])
                else:
                    tmp["context"] = ' '.join(self.contexts[i] for i in doc_indices[idx])
###
