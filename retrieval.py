...
from utils_qa import clean_text
...
class SparseRetrieval:
    def __init__(self, tokenize_fn, data_path="/opt/ml/input/data/data/", context_path="wikipedia_documents.json"):
        ...
        # self.contexts = list(map(clean_text, self.contexts))
    ...
    def retrieve(self, query_or_dataset, topk=30, algorithm='TF-IDF'):
        if algorithm == 'TF-IDF':
        ...
        if isinstance(query_or_dataset, str):
            ...
            with timer("query exhaustive search"):
                doc_scores, doc_indices = (self.get_relevant_doc_bulk if algorithm == 'TF-IDF' else self.get_relevant_doc_bulk_bm25)(query_or_dataset['question'], k=topk)
            for idx, example in enumerate(tqdm(query_or_dataset, desc="Sparse retrieval: ", bar_format=bar_format)):
                ...
                tmp = {
                    ...
                    # "context": all_context # retrieved doument
                }

                if 'context' in example.keys():
                    tmp['context'] = example['context'] + '\n'.join(self.contexts[i] for i in doc_indices[idx] if self.contexts[i] != example['context'])
                else:
                    tmp['context'] = '\n'.join(self.contexts[i] for i in doc_indices[idx])
    ...
    def get_relevant_doc(self, query, k=10):
        """
    ...
    def get_relevant_doc_bulk_bm25(self, queries, k=1):
        if self.tokenized_contexts is None:
            self.tokenized_contexts = [self.tokenize(context) for context in tqdm(self.contexts, desc='bm25 tokenizing', bar_format=bar_format)]

        bm25 = BM25Okapi(self.tokenized_contexts)

        doc_scores = []
        doc_indices = []
        for query in tqdm(queries, desc='BM25 점수 계산 중', bar_format=bar_format):
...
class DenseRetrieval:
    ...
    def __init__(self, data_path="/opt/ml/input/data/data/", context_path="wikipedia_documents.json"):
        self.data_path = data_path
        with open(os.path.join(data_path, context_path), "r") as f:
            wiki = json.load(f)
        ...
        self.p_encoder.load_state_dict(model_dict['p_encoder'])
        self.q_encoder.load_state_dict(model_dict['q_encoder'])
    ...
    def retrieve(self, query_or_dataset, topk=30):
        assert self.p_embedding is not None, "You must build faiss by self.get_sparse_embedding() before you run self.retrieve()."
        if isinstance(query_or_dataset, str):
            doc_scores, doc_indices = self.get_relevant_doc(query_or_dataset, k=topk)
            print("[Search query]\n", query_or_dataset, "\n")
