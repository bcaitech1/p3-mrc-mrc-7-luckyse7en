...
from utils_qa import clean_example, postprocess_qa_predictions, check_no_error, tokenize, clean_text
...
def main():
    ...
    # training_args.output_dir = f'{training_args.output_dir}epochs_{training_args.num_train_epochs}/'
    ...
    if data_args.train_retrieval:
        datasets = run_sparse_embedding(tokenizer.tokenize, datasets, training_args, retrieval_args, topk=10)
...
def run_sparse_embedding(tokenize, datasets, training_args, retrieval_args, topk=1):
    retriever = SparseRetrieval(tokenize_fn=tokenize,
                                data_path="/opt/ml/input/data/data/",
                                context_path="train_wiki.json")
                                
    if retrieval_args.retrieval_algorithm == 'TF-IDF':
        retriever.get_sparse_embedding()
    ######

    train_df = retriever.retrieve(datasets['train'], topk=topk, algorithm=retrieval_args.retrieval_algorithm)
    val_df = retriever.retrieve(datasets['validation'], topk=topk, algorithm=retrieval_args.retrieval_algorithm)
...
def run_mrc(data_args, training_args, model_args, datasets, tokenizer, model):
    ...
    def prepare_train_features(examples):
        # 나의 전처리
        # examples = clean_example(examples, [question_column_name, context_column_name])
