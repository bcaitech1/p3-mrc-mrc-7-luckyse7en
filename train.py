...
def main():
    ...
    datasets['train'] = datasets['train'].map(data_change)
    datasets['validation'] = datasets['validation'].map(data_change)
    print(datasets)
...
def data_change(examples):
    text = {}
    answer = examples['answers']['text'][0]
    answer_start, answer_len = int(examples['answers']['answer_start'][0]), len(answer)
    split_text = examples['context'][:answer_start+1]
    split_text = prepro.prepro([split_text])[0][:-1]
    answer = prepro.prepro([answer])[0]
    text['answers'] = {'answer_start': [len(split_text)], 'text': [answer]}
    text['context'] = prepro.prepro([examples['context']])[0]
    text['question'] = prepro.prepro([examples['question']])[0]
    return text
