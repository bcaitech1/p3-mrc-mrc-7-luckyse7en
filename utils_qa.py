...
def postprocess_qa_predictions(
    ...
    #     all_nbest_json[example['id']].sort(key=lambda pred: pred['score'], reverse=True)
    #     all_nbest_json[example['id']] = all_nbest_json[example['id']][:n_best_size]
    
    # all_predictions = {example_id: pred[0]['text'] for example_id, pred in all_nbest_json.items()}
...
def clean_example(examples, cols):

    for col in cols:
        examples[col] = [clean_text(data) for data in examples[col]]
